...
class RelationUpdater {
    ...
    private fun mutateRelation(relationUpdate: RelationUpdate, links: MutableList<Link>) =
        when (relationUpdate.operation) {
            RelationOperation.ADD -> addLinks(links, relationUpdate.relation.links)
            RelationOperation.DELETE -> deleteLinks(links, relationUpdate.relation)
        }

    fun addLinks(target: MutableList<Link>, toAdd: List<Link>) =
        toAdd.forEach { link ->
            if (target.none { linkMatches(it, link) })
                target.add(link)
        }

    fun addLinks(targetResource: FintResource, relation: String, toAdd: List<Link>) =
        getRelationLinks(relation, targetResource)?.let { target ->
            toAdd.forEach { link ->
                if (target.none { linkMatches(it, link) })
                    target.add(link)
            }
