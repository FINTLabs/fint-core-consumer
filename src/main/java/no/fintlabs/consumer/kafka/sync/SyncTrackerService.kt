        ...
        timed(resourceName, syncType, "sync.processRecordMetadata") {
            val previousSyncState =
                timed(resourceName, syncType, "sync.state.load") {
                    syncCache.get(correlationId) { Init(resourceName, consumerRecordMetadata.totalSize, syncType) }!!
                }
            val newSyncState =
                timed(resourceName, syncType, "sync.state.transition") {
                    previousSyncState.transition(resourceName, consumerRecordMetadata.totalSize)
                }
