...
class RelationUpdateProducerTest {
    ...
    private val template: ParameterizedTemplate<RelationState> = mockk()
    ...
    private fun newProducer(future: CompletableFuture<SendResult<String, RelationState>>): RelationUpdateProducer {
        every { factory.createTemplate(RelationState::class.java) } returns template
        every { template.send(any<ParameterizedProducerRecord<RelationState>>()) } returns future
    ...
    fun `records published when send completes successfully`() {
        val sendResult: SendResult<String, RelationState> = mockk(relaxed = true)
        val producer = newProducer(CompletableFuture.completedFuture(sendResult))

        producer.publish(state, "elev", "elev-1").join()

        verify(exactly = 1) { kafkaThroughputMetrics.recordRelationStateProduced("elevfravar", "published") }
        verify(exactly = 0) { kafkaThroughputMetrics.recordRelationStateProduced(any(), "failed") }
    }

    @Test
    fun `records failed when send completes exceptionally`() {
        val failed: CompletableFuture<SendResult<String, RelationState>> = CompletableFuture()
        failed.completeExceptionally(RuntimeException("broker unavailable"))
        val producer = newProducer(failed)

        runCatching { producer.publish(state, "elev", "elev-1").join() }

        verify(exactly = 1) { kafkaThroughputMetrics.recordRelationStateProduced("elevfravar", "failed") }
        verify(exactly = 0) { kafkaThroughputMetrics.recordRelationStateProduced(any(), "published") }
    }
}
